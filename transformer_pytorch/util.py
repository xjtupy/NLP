...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, model_dim=512, num_heads=8, dropout=0.0):
        """
        多头attention机制
        :param model_dim: 第一层word embedding维度
        """
    ...
    def forward(self, key, value, query, attn_mask=None):
        """
        key/value/query的维度为（input/target_length, emb_dim）
        """
        # 残差连接
        residual = query
...
def padding_mask(seq_k, seq_q):
    """
    用于encoder和decoder，batch数据等长，不足的补齐，计算loss的时候忽略掉补齐的部分
    :param seq_k:
    :param seq_q:
    :return:
    """
...
def sequence_mask(seq):
    """
    用于decoder，只关注序列以前的信息，mask 以后的信息
    :param seq:
    :return:
    """
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, input_len):
        ...
        input_pos = tensor([list(range(1, int(len) + 1)) + [0] * (max_len - len) for len in input_len])
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, inputs_len, enc_output, context_attn_mask=None):
        ...
        # 大于 0 的地方为padding_mask和seq_mask一方或都要mask的地方
